...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ref={ref}
      className={cn("fixed top-3 left-0 w-full z-50", className)}
    >
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-transparent px-4 py-2 lg:flex dark:bg-transparent-500",
        visible,
        className,
      )}
    >
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-blue-400 dark:bg-neutral-800"
            />
          )}
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-white/10 dark:bg-transparent-500",
        className,
      )}
    >
      {children}
    ...
    <div className="flex flex-col w-full space-y-2">
      {items.map((item, idx) => (
        <a
          ...
          href={item.link}
          onClick={onItemClick}
          className="block w-full rounded-md px-4 py-2 text-white hover:bg-white/10 dark:text-white"
        >
          {item.name}
        </a>
      ))}
  ...
  return (
    ...
    >
      <img
        ...
        height={30}
      />
      <span className="font-medium text-white dark:text-white">Lance</span>
    </a>
  );
};
